...
+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        Method oldMethod = class_getInstanceMethod(objc_getClass("__NSArrayM"), @selector(objectAtIndex:));
        Method newMethod = class_getInstanceMethod(objc_getClass("__NSArrayM"), @selector(swizzled_objectAtIndex:));
        // 交换方法
        method_exchangeImplementations(oldMethod, newMethod);
    });
}
